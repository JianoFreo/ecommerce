...
public class ProductDAO { // DAO class for database operations
    ...
    public void addProduct(Product product) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO products (name, description, costPrice, price, quantity, categoryID, imageData) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    public void updateProduct(Product product) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                "UPDATE products SET name=?, description=?, costPrice=?, price=?, quantity=?, categoryID=?, imageData=? WHERE id=?");
            ps.setString(1, product.getName());
            ps.setString(2, product.getDescription());
            ps.setDouble(3, product.getCostPrice());
            ps.setDouble(4, product.getPrice());
